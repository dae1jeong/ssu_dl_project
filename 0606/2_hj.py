...
class CombinedModel(nn.Module):
    def __init__(self, num_classes=100):
        ...
        # ✅ 여기 추가
        self.efficient.conv_stem = nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1, bias=False)
        self.efficient.bn1 = nn.BatchNorm2d(32)
        self.efficient.blocks[0][0].stride = (1, 1)       
...
for epoch in range(epochs):
    if epoch == freeze_epoch:
        print(f"🔓 Epoch {epoch}: Backbone fine-tuning 시작")
...
if best_model_state is not None:
    model.load_state_dict(best_model_state)     # best 모델 불러오기
    swa_model.update_parameters(model)          # SWA 모델에 적용

# SWA 저장
update_bn(train_loader, swa_model, device=device)
torch.save(swa_model.module.state_dict(), "weight_가반4조_0606_여러수정.pth")
...
def tta_tf_vflip(x): return torch.flip(x, dims=[1])
def tta_tf_rotate90(x): return x.rot90(1, [1, 2])
def tta_tf_rotate270(x): return x.rot90(3, [1, 2])
tta_transforms = [
    tta_tf_identity,
    tta_tf_hflip,
    tta_tf_vflip,
...
with open("results_가반4조_0606_여러수정.txt", "w") as f:
