...
def eval_acc(model, data_loader, device):
    ...
    with torch.no_grad():
        for batch in data_loader:
            # CustomImageDataset와 CIFAR100의 반환값 다름
            images = batch[0].to(device)
            labels = batch[1]
            if torch.is_tensor(labels):
                labels = labels.to(device)
            else:
                continue  # CustomImageDataset에서는 label이 없음
...
torch.save(swa_model.module.state_dict(), "weight_가반4조_0602_SWA.pth")
...
with torch.no_grad():
    for images, filenames in tqdm(test_loader, desc="Testing"):
        ...
        for fname, pred in zip(filenames, preds):
            ...
            results.append((number, int(pred)))

# number 기준 오름차순 정렬
results.sort(key=lambda x: x[0])

# 저장
with open("results_가반4조_0602_SWA.txt", "w") as f:
    f.write("number, label\n")
    for number, label in results:
        f.write(f"{str(number).zfill(4)}, {label}\n")

print("results_가반4조_0602_SWA.txt 저장 완료 (정렬 포함)")