...
# bias, bn, norm 계열은 weight_decay 제외
decay = []
no_decay = []
...
scheduler = OneCycleLR(
    optimizer,
    max_lr=0.005,  # 실험적으로 0.003~0.01 등 여러 값 시도 가능
...
for epoch in range(epochs):
    ...
    for images, labels in tqdm(train_loader, desc=f"Epoch {epoch+1}"):
        ...
        loss.backward()
        optimizer.step()
        scheduler.step()  # OneCycleLR는 미니배치마다 step
...
# 가중치 저장
torch.save(model.state_dict(), "weight_가반4조_0602_2000.pth")

# 테스트셋 추론 및 저장
model.load_state_dict(torch.load("weight_가반4조_0602_2000.pth"))
model.eval()

with open("results_가반4조_0602_2000.txt", "w") as f:
    ...
    with torch.no_grad():
        for images, filenames in tqdm(test_loader, desc="Testing"):
            ...
            for fname, pred in zip(filenames, preds):
                ...
                f.write(f"{number.zfill(4)}, {int(pred):02d}\n")

print("✅ results_가반4조_0602_2000.txt 저장 완료")
